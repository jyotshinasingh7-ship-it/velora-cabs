...
- Unit 003A fare lock/lifecycle separation is locally/emulator verified and deployed to Vercel Preview with matching Firestore rules on `velora-cabs-staging`; anonymous `/`, `/admin/analytics`, and protected-dispatch smoke checks pass. Authenticated lifecycle/browser verification remains pending. On Windows, Vercel CLI 54.20.1/56.3.1 adapters misclassify the valid static `/admin/analytics` output as requiring a missing lambda and stop before producing functions/static output, while the Vercel Linux builder packages the same Next.js 16.2.10 route successfully. Actual payment, commission, wallet, cash due, withdrawals, refunds, and release automation remain unimplemented.
