...
- The broad test seed remains available. A dedicated Unit 003A staging-only seed now enforces exact project targeting, minimal customer/driver/admin fixtures, ignored credential storage, and cleanup; live staging creation is blocked because the current Preview Admin identity lacks permission on `velora-cabs-staging`.
...
- 2026-07-17 Unit 003A staging identity preparation: dedicated seed syntax/dry-run and the exact production-target refusal passed. Live execution against explicit `velora-cabs-staging` failed closed with `auth/insufficient-permission` before any Auth/Firestore fixture write because the linked Preview Admin identity is not authorized for staging. The unused local credential artifact was removed; no passwords were printed. Focused test, typecheck, build, diff, ignored-path, and secret checks passed; lint had zero errors and the same four warnings. Production was untouched and authenticated browser verification remains pending.
- 2026-07-17 Unit 003A staging identity retry: after the owner-reported Admin credential correction, value-free Preview diagnostics still showed a non-staging injected project identity. The explicit staging override passed the target guard, but sequential access verification returned Firebase Auth `auth/insufficient-permission`; the prior concurrent preflight also returned Firestore `PERMISSION_DENIED`. Firebase CLI confirmed the staging database exists. No identity, fixture, password file, staging data, or production data was created/changed. Preview Admin IAM/stale-variable delivery remains the blocker. Seed dry-run, focused tests, typecheck, build, and diff checks passed; lint had zero errors and the same four warnings.
- 2026-07-17 Unit 003A successful staging seed: validated the single staging service-account file without exposing values, updated only branch-scoped Sensitive Vercel Preview Admin variables, and hydrated the ignored local Preview file because Vercel does not pull Sensitive values. Staging identity, Auth, and Firestore preflight passed. The seed confirmed one verified customer, one approved/active initially offline driver, one active admin, approved application/vehicle, supported services, and isolated pricing. Credentials exist only in the ignored test-credential path; production and Unit 003B were untouched. Authenticated browser verification remains pending.
...
- 2026-07-17 — Unit 003A staging fixture preparation — Audited the broad seed and added an exact-project, staging-only minimal customer/driver/admin fixture workflow with ignored file-based credentials and cleanup. Syntax, dry-run, and production refusal passed. The live staging call failed before mutation with `auth/insufficient-permission`; the Preview Admin identity cannot access `velora-cabs-staging`. Removed the unusable local credential artifact, left production untouched, and kept Unit 003A active. Unit 003B was not started.
- 2026-07-17 — Unit 003A staging fixture retry — Retried after the reported Preview Admin correction. Admin variable presence passed, but Preview project identity still mismatched and explicit staging access failed for both Auth and Firestore permissions before mutation. Added independent value-free access diagnostics, confirmed the staging Firestore database exists, retained the ignored/absent credential file state, and left production untouched. Unit 003B was not started.
- 2026-07-17 — Unit 003A staging fixture success — Securely applied the validated staging service account only to the Unit 003A Preview branch, loaded the ignored Preview env explicitly, passed staging identity/Auth/Firestore gates, and seeded/verified the minimal customer, driver, admin, driver application, vehicle, service, and pricing fixtures. Passwords were not printed; the credential file remains ignored. Production was untouched and Unit 003B was not started.
